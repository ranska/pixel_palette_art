# spec/pixel_palette_full_spec.py
#
# Specs réactivées depuis spec_pending/pixel_palette_spec.py.pending
# après restauration du constructeur complet de PixelPalette.
#
# Tests encore en attente (nécessitent des corrections dans les parsers/exporters) :
# - parse les métadonnées GIMP (le parser ne remplit pas palette.metadata)
# - parse le format hexadécimal générique (le parser générique ne parse que 1/4 des hex)
# - formate en RGB avec nom de palette issu du parsing
# - formate en raw (format d'export inexistant)
# - exporte en format GIMP sans header (to_formatted_string ajoute un header)

from spec.conftest import *

from mamba import description, context, it, before
from expects import (expect, be_empty, have_length, equal, be_true, be_false,
                     contain, start_with, end_with, raise_error, be_above_or_equal,
                     be_below_or_equal)
...
with description('PixelPalette initialisation') as self:
...
"""
        ...
        # TODO: parser ne remplit pas palette.metadata (issue #1)
...
"""
    ...
    with context('Parsing formats génériques'):
        # TODO: generic parser ne parse pas tous les formats hex (issue #1)
...
with description('PixelColor validation') as self:
    with context('Classe PixelColor'):
        with it('valide les valeurs RGB'):
            ...
            expect(color_clamped.r).to(equal(255))
            expect(color_clamped.g).to(equal(0))
            expect(color_clamped.b).to(equal(255))
        ...
        with it('calcule la distance entre couleurs'):
            ...
            # sqrt(255^2 + 255^2) = 360.6244...
            expect(distance_rouge_vert).to(be_above_or_equal(360.62))
            expect(distance_rouge_vert).to(be_below_or_equal(360.63))
            # sqrt(5^2 + 5^2 + 5^2) = sqrt(75) ≈ 8.66
            expect(distance_rouge_proche).to(be_above_or_equal(8.66))
            expect(distance_rouge_proche).to(be_below_or_equal(8.67))


with description('PixelPalette utilitaires') as self:
    with context('Méthodes utilitaires'):
        with before.each:
            ...
            self.palette.add_color(255, 0, 0, "Rouge Dupliqué")
        ...
        with it('supprime des couleurs'):
            success = self.palette.remove_color(1)
        ...
        with it('retourne les couleurs uniques'):
            ...
            expect(unique).to(have_length(2))


with description('PixelPalette formatage') as self:
...
with description('PixelPalette export') as self:
    with context('Export formats'):
        ...
        with it('exporte en liste hex'):
            result = self.palette.to_list("hex")
...
with description('PixelPalette propriétés') as self:
    with context('Propriétés et comportement'):
        ...
        with it('implémente les propriétés de base'):
            palette = PixelPalette(name="Test")
...
with description('PixelPalette erreurs') as self:
...
"""
        ...
        with it('trouve la couleur proche sur palette vide lève une erreur'):
            ...
            expect(find_closest).to(raise_error(ValueError))
